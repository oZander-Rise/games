...
(function (Ω) {
	...
	input = {
		...
		isTouchInRect: function (x1, y1, x2, y2) {
			return (touch.x >= x1 && touch.y >= y1 && touch.x <= x2 && touch.y <= y2);
		},

		isMouseInRect: function (x1, y1, x2, y2) {
			return (mouse.x >= x1 && mouse.y >= y1 && mouse.x <= x2 && mouse.y <= y2);
		},

		isCoordsInRect: function (x1, y1, x2, y2) {
			return this.touch.x ? this.isTouchInRect(x1, y1, x2, y2) : this.isMouseInRect(x1, y1, x2, y2);
		}
	};
	...
	function bindMouse() {
		...
		function setPos(e) {
			...
			var relX = (e.pageX - rect.left) / game.rate,
				relY = (e.pageY - rect.top) / game.rate;
...
(function (Ω) {
	...
	if (window.AudioContext || window.webkitAudioContext) {
		if (window.AudioBufferSourceNode) {
			...
			p.noteOn = p.noteOn || p.start;
			p.noteOff = p.noteOff || p.stop;
		}
		...
		Sound = Ω.Class.extend({
			...
			init: function (path, volume, loop) {
				...
				if (!sounds[path]) {
					if (window.sounds[path]) {
						this._decodeBuffer({target: {response: window.sounds[path]}});
					} else {
						var xhr = new XMLHttpRequest,
							src = path.slice(-4).slice(0, 1) === "." ? path : path + this.ext;
						xhr.open("GET", src);
						xhr.responseType = "arraybuffer";
						xhr.addEventListener("load", this._decodeBuffer.bind(this), false);
						xhr.send();
					}
				} else
					this._setSource();
			...
			_decodeBuffer: function(event) {
				var xhr = event.target;
				if (!context.decodeAudioData) {
					sounds[this.path] = context.createBuffer(xhr.response, false);
					this._setSource();
				} else
					context.decodeAudioData(xhr.response,
						this._setBuffer.bind(this),
						this._resolve.bind(this)
						);
			},

			_setBuffer: function(buffer) {
				sounds[this.path] = buffer;
		...
		Sound = Ω.Class.extend({
			...
			play: function () {
				if (window.game && !window.game.soundOn)
					return;
...
(function (Ω) {
	...
	function isPointInRect(m, a, b, c) {
		var ab = new Point(b.x - a.x, b.y - a.y),
			bc = new Point(c.x - b.x, c.y - b.y),
			am = new Point(m.x - a.x, m.y - a.y),
			bm = new Point(m.x - b.x, m.y - b.y),
			d1 = dot(ab, am),
			d2 = dot(bc, bm);

		return 0 <= d1 && d1 <= dot(ab, ab)
			&& 0 <= d2 && d2 <= dot(bc, bc);
	}

	function isEllipseInSector(e, a, b) {
		var x0 = e.x,
			y0 = e.y,
			r1 = e.rx,
			r2 = e.ry,
		...
		if (x1 == x2) {
			resX1 = resX2 = x1;
			d = Math.sqrt(r1*r1*r2*r2*(r1*r1-x0*x0+2*x0*x1-x1*x1));
			resY1 = (d+r1*r1*y0)/(r1*r1);
			resY2 = (-d+r1*r1*y0)/(r1*r1);
		} else {
			d = Math.sqrt(r1*r1*r2*r2*(r2*r2-y0*y0+2*y0*y2-y2*y2));
			resX1 = (d+r2*r2*x0)/(r2*r2);
			resY1 = (resX1*y1-resX1*y2+x1*y2-x2*y1)/(x1-x2);
			resX2 = (-d+r2*r2*x0)/(r2*r2);
			resY2 = (resX2*y1-resX2*y2+x1*y2-x2*y1)/(x1-x2);
		}

		if (!isNaN(resX1 + resY1) && ((resX1 >= x1 && resX1 <= x2)
			|| (resX1 >= x2 && resX1 <= x1))
			&& ((resY1 >= y1 && resY1 <= y2)
			|| (resY1 >= y2 && resY1 <= y1)
			))
		...
		if (!isNaN(resX2 + resY2) && ((resX2 >= x1 && resX2 <= x2)
			|| (resX2 >= x2 && resX2 <= x1))
			&& ((resY2 >= y1 && resY2 <= y2)
			|| (resY2 >= y2 && resY2 <= y1)
			))
	...
	var Physics = {
		...
		checkEllipseCollision: function(entity, entities, cbName) {
			cbName = cbName || "hit";
			...
			for (var i = 0; i < len; ++i) {
				...
				pa = new Point(b.x, b.y);
				pb = new Point(b.x + b.w, b.y);
				pc = new Point(b.x + b.w, b.y + b.h);
				pd = new Point(b.x, b.y + b.h);

				if (isPointInRect(a, pa, pb, pc)
					|| isEllipseInSector(a, pa, pb)
					|| isEllipseInSector(a, pb, pc)
					|| isEllipseInSector(a, pc, pd)
					|| isEllipseInSector(a, pd, pa)) {
					a[cbName] && a[cbName](b);
					b[cbName] && b[cbName](a);
				}
			}
...
(function (Ω) {
    ...
    var SpriteAtlas = Ω.Class.extend({
        ...
        render: function (gfx, name, x, y, angle) {
            x = (x + 0.5) | 0;
            y = (y + 0.5) | 0;
            ...
            if (angle) {
                ...
                var context = gfx.ctx;
                context.mozImageSmoothingEnabled = context.webkitImageSmoothingEnabled = true;

                gfx.ctx.translate(x + imgData.w / 2, y + imgData.h / 2);
        ...
        renderCustom: function (gfx, name, x, y, w, h, dy) {
            x = (x + 0.5) | 0;
            y = (y + 0.5) | 0;
